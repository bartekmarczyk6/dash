...
if [ -f /etc/os-release ]; then
  ...
  if [ $OS_DISTRO = "Debian" ]; then
  ...
  elif [ $OS_DISTRO = "Ubuntu" ] || [ $OS_DISTRO = "Zorin" ]; then
    isUbuntu=true
    UBUNTU_VERSION=$(source /etc/os-release; echo ${VERSION_ID%% *} | cut -c1-2)
    if (( $UBUNTU_VERSION >= 22 )); then
      echo "Detected Ubuntu 22.04 (Jammy) or newer"
      JAMMY=true
    else
    ...
    echo "Unsupported OS detected. Recommended Debian 12, Ubuntu 22.04, or a derivative."
    exit 1
  fi
fi

# This is an x86 build, so Raspberry Pi specific flags are not needed.
installArgs=""
...
if [ $dash = false ]; then
  ...
  if [[ $choice == "y" || $choice == "Y" ]]; then
    ...
    if [[ $? -eq 1 ]]; then
     ...
     echo "Exiting. Check autostart.sh for more options"
     exit 0
  fi
fi
